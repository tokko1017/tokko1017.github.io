...
ページ上部のバナーは `images/banner.jpg` の1枚絵です。タイトル文字も画像に含める前提のため、
差し替えるだけで見た目が変わります。

- **推奨サイズ**: 幅1600px × 高さ400px（横4:1の比率）
- この比率であれば、PC・スマホどちらでも画像全体が欠けずに横幅いっぱいに表示されます
- `images/banner.jpg` という同じファイル名で上書き保存すれば、コードの変更は不要です
  （ファイル名を変える場合は `index.html` 内の `<img class="banner-image" src="images/banner.jpg" ...>` も書き換えてください）
