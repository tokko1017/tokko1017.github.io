...
ヘッダー（バナー）は `images/banner.jpg` の1枚絵をそのまま表示する方式。
タイトル文字・紹介文はすべて画像側に含まれている想定のため、`index.html` の
`h1#site-title` / `p#intro-text` は画面上は非表示（`.sr-only`）にしてSEO/読み上げ用にのみ残している。
バナーを差し替えたい場合は `images/banner.jpg` を上書きするだけでよい（推奨サイズ: 幅1600×高さ400px、4:1比率）。
...
- 外部ライブラリ・CDN読み込みは追加しない（オフラインでも壊れないシンプル構成を維持）。
  例外: Google Fonts（Zen Maru Gothic）のみ、見た目のための追加読み込みを許可している。
  読み込めない場合はシステムフォントにフォールバックし、機能は壊れない。

## 5. 本を追加する手順（このリポジトリでの正しい手順）
`README.md` の「2. 本を1冊追加する手順」を参照。要約すると:
1. `images/` に表紙を追加
2. `js/books-data.js` の `books` 配列の**4番目（最新刊の指定席）**を新刊に入れ替え、
   それまで4番目にあった本は5番目（先頭）に移動する（1〜3番目の売れ筋固定枠は動かさない）
